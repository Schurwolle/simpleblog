	...
	public function validateCKEImgs($attribute, $value, $parameters, $validator)
	{
		if(preg_match_all('#<a href="[^<>"]*"[^<>]*><img [^<>]*src="[^<>"]*"[^<>]*/></a>#', $value, $matches))
        {
            ...
            {                
                ...
                if(starts_with($photo, '/pictures/') && (!file_exists(base_path().'/public'.$photo)) || starts_with($thumb[1], '/pictures/') && (!file_exists(base_path().'/public'.$thumb[1])) || ((!starts_with($photo, '/pictures/')) && ((!filter_var($photo, FILTER_VALIDATE_URL)) || (!getimagesize($photo)))) || ((!starts_with($thumb[1], '/pictures/')) && ((!filter_var($thumb[1], FILTER_VALIDATE_URL)) || (!getimagesize($thumb[1])))))
                {
                	return false;
                }
            }
