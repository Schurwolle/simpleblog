							...
							@unless ($article->tags->isEmpty())
								<h5>Tags: 
									@foreach($article->tags as $tag)
										<a href="/tags/{{ $tag->name }}"><button class="btn btn-default btn-xs @if($tag->name == strtolower(implode($query_words))) marker @else @foreach($new_query_words as $word) {{ $tag->name == strtolower($word) ? 'marker' : '' }}@endforeach @endif"> {{ $tag->name }} </button></a>
									@endforeach
								</h5>
							@endunless
						</td>
											...
											<div class="panel panel-default">
												...
												<div class="panel-body">
													<a href="articles/{{$article->slug}}/search/{{$query_link
														...
														<button class="btn btn-info">{{ str_word_count(strip_tags($comment->body)) > $i ? 'See Full Comment' : 'Go To Comment'}}</button>
													</a>
												</div>
									        </div>	
										</div>	
