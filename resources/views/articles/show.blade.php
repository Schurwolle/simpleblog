	...
	@unless ($article->tags->isEmpty())
		<h5>Tags: 
			@foreach($article->tags as $tag)
				<a href="/tags/{{ $tag->name }}"><button class="btn btn-default btn-xs @if(isset($query_words)) @if ($tag->name == strtolower(implode($query_words))) marker @else @foreach($new_query_words as $word) {{ $tag->name == strtolower($word) ? 'marker' : '' }}@endforeach @endif @endif"> {{ $tag->name }} </button></a>
			@endforeach
		</h5>
	@endunless
	<br>
	...
	<script>
		...
		function showFull() {
			...
			if(commentPanel.innerHeight() < 300)
			{
				...
				if (!commentPanel.siblings('.panel-body').find('button').length)
				{
					commentPanel.siblings('.panel-body').remove();
				}
				return;
			}
			$(this)
